...
3. 클리핑(이미지를 자르거나 축소)
- 전체 맵에서 내 시야(카메라 영역)만 잘라서 렌더링 하겠다.
- 미니맵
...
6. 각각 따로 배경 넣어주기
6.1 여기에서 psd파일로 백업
7. 이미지 크기 기준 장수 곱해서 캔버스 크기 설정
7.1 왼쪽 기준으로 늘리기 설정
7.2 픽셀로
8. 캔버스에서 이동도구로 이동시켜서 붙히기
9. 세로픽셀 두배로 크기 설정
10. 모든 레이어 선택후 병합.
11. 복사
12. ctrl + 우클릭 회전
...
#pragma region 과제0903
/*
개인 조사 : 함수 오버로딩 7규칙

한줄 조사. CreateCompatibleDC / CreateCompatibleBitmap 을 포함한 Image관련 함수를 조사해올것.

※ 이번 과제부터 OOP에 기반하여 과제를 해올것
...
- 예외처리: 두더지를 한번 클릭 -> 점수 증가 -> 이후 두더지를 클릭할때마다 점수가 오르면 안된다(두더지가 사라질때까지)

과제 2. 클레이 사격

- 좌 / 우에서 무작위로 출현하는 타겟을 마우스로 제거

- 타겟 제거시 점수 획득

- 제거된 타겟은 그 자리에서 바로 떨어져야 한다.

- 등장하는 타겟의 종류는 무작위 도형처리를 해올것
- 사각형:50점 / 원:-100점 / 다각형:100점

- 시간은 60초동안 지속 / 시간이 끝나면 다시 시작할 수 있는 방안을 강구할 것

※ 충돌함수 봉인 해제

과제 3. 1번 + 2번 과제에 이미지 추가해 올것

- 배경 + 단일 이미지를 추가해오면 된다.

- EX : 두더지 없을때, 두더지 나올때, 두더지 때렸을때, 망치 대기상태, 망치 때릴때

*/
#pragma endregion

#pragma region 과제0904
/*
과제1. 슬라이스 게임
...
b. 캐릭터 (시리즈)
- 시작 씐 + 게임 씐
- 게임 씐 -> 아래의 이미지를 GUI화 시켜 (혹은 버튼화 시켜) 재생시킨다. (프레임)
- 포함해야 할 이미지
-> 배경, 대기, 이동(좌/우), 찌르기(좌,우), 대각선 찌르기, 연속 찌르기(좌/우), 원 돌리기, 승리 포즈(옷 던지기), 스킬 클라이막스 연출
