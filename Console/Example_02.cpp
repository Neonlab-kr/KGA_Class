...
- float (0.0000000)(4바이트)
- double (14에서 16자리)(8바이트)

※ decimal(C#에만 포함됨 C++에 없음)(쓰지마 그냥!!!)(소숫점 29자리까지)(16바이트)
...
ㄴ bool 자료형은 정수가 와야 할 위치에 오게 되면 각각 1과 0으로 반환이 될뿐 각각 정수를 의미하는게 아니다.


- 기본적으로 C 언어는 논리형에 해당하는 참 / 거짓이라는 자료형이 별도로 존재하지 않았다.
ㄴ 그렇기 때문에 정수 0을 거짓 / 0을 제외한 정수는 참으로 인지를 시켰다.
ㄴ 단, 현재는 C언어에도 자료형이 존재하니 헷갈리지 말것

- C++로 넘어오면서 참 또는 거짓을 나타내는 키워드를 추가했고 이게 true / false가 되겠다.

- 그리고 C / C++ 자료형은 기본적으로 음수를 표현하는 것이 가능하지만 경우에 따라서 음수를 표현하지 않는
대신에 양수를 좀 더 크게 표현하는 방법이 있다(signed (음) / unsigned (양))
...
void main()
{
	...
	- 예전 절차지향적인 방식으로 코딩을 할 때 많이 사용된 표기법
		- 이 변수가 어떤 변수인지 알기 위하여 서로 약속하듯이 앞에 그 변수 형태가 뜻하는 약자를 붙인다.
		- EX
		int num			<->		nNum;
	...
	*/
	...
	// RAND_MAX
	// 32767 이하의 랜덤은 이걸 사용(비용이 적음)
	// 불완전한 난수
	// 더 정밀하고 고르게 분포된 함수는 C++의 함수를 사용
